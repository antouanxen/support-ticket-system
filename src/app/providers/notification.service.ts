...
export class NotificationService {
    ...
    public async spreadNotifications(action: Notification_action | Notification_ownAction, customTicketId: string, userId: string) {

        // βρισκω τους υπολοιπους χρηστες αποδεκτες 
        const usersListeners = await this.getUsersToNotify(action as Notification_action, userId)

        // βρισκω το χρηστη που κανει την ενεργεια αναλογα το userId
        const userWhoActed = await prisma.user.findUnique({ where: { id: userId } })
        ...
        const notifications = await Promise.all(usersListeners.map(async (userListener) => {
            let content: string
            ...
            if (Object.values(Notification_ownAction).includes(action as Notification_ownAction)) {
                content = await this.createNotificationOwnContent(action as Notification_ownAction, customTicketId)
                ...
                content = await this.createNotificationContent(action as Notification_action, customTicketId, userWhoActed.id)
            }
    
            // φτιαχνω στη βαση το notification, σε περιπτωση που θελω να το κανω fetch στο μελλον
            const newNotification = await prisma.notification.create({
                data: {
                    action: userId ? action as Notification_action : action as Notification_ownAction,
                    content: content,
        ...
        const clearNotifications = notifications.filter(notification => notification !== null)
    ...
    public async createNotificationContent(action: Notification_action, customTicketId: string, userId: string) {
        let content = ''

        // βρισκω τον χρηστη, αλλα και το ticket απο το customTicketId
        const user = await prisma.user.findUnique({ where: { id: userId }, select: { userName: true } })
        const ticket = await prisma.ticket.findUnique({ where: { customTicketId: customTicketId }, select: { customTicketId: true } })
    ...
    public async createNotificationOwnContent(action: Notification_ownAction, customTicketId: string) {
        let content = ''
    ...
    public async getUsersToNotify(action: Notification_action, userId: string) {
        ...
        if (action === Notification_action.REMOVED_ENGINEER) {
            const permittedUsers = usersToNotify.filter(user => user.role.role_description === AuthRoles.ADMIN)
